...
import { CheckCircle2, ChevronRight, LockKeyhole, PlayCircle, Receipt, ShieldCheck, Wallet } from 'lucide-react'
import type {
  CurrencyCode,
  GivingFrequency,
  GivingProps,
...
export function GivingHubWizard({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-primary-50/70 via-white to-secondary-50/50 dark:from-neutral-950 dark:via-neutral-950 dark:to-neutral-900">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-8">
        <section className="rounded-3xl border border-primary-200/70 dark:border-primary-400/40 bg-white/90 dark:bg-neutral-900 p-5 sm:p-6 shadow-sm">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary-100 dark:bg-primary-500/20 px-3 py-1 text-xs font-semibold text-primary-800 dark:text-primary-100">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            <div className="rounded-2xl border border-neutral-200 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-800 px-4 py-3">
            ...
            <div className="rounded-2xl border border-secondary-200 dark:border-secondary-400/40 bg-secondary-50/80 dark:bg-secondary-500/10 px-4 py-3">
              <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-secondary-800 dark:text-secondary-100">
                <LockKeyhole className="h-3.5 w-3.5" strokeWidth={1.8} />
                {data.securityBadgeText}
              </p>
              <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-100">
          ...
          {onOpenHistory && (
            <div className="mt-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3 py-2 text-xs font-semibold text-neutral-700 dark:text-neutral-200 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors"
              >
        ...
        <section className="mt-5">
          <h2 className="text-lg font-semibold text-neutral-900 dark:text-neutral-50">Impact Goals</h2>
              ...
              return (
                <button
                  ...
                  className={`rounded-3xl border p-4 text-left transition-all duration-200 ${
                    isSelected
                      ? 'border-primary-300 dark:border-primary-400/60 bg-primary-50 dark:bg-primary-500/10 shadow-md shadow-primary-500/10'
                      : 'border-neutral-200 dark:border-neutral-600 bg-white/85 dark:bg-neutral-900 hover:-translate-y-0.5 hover:shadow-sm'
                  }`}
                >
                  <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{fund.title}</p>
                  <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-200 line-clamp-2">{fund.description}</p>
                  ...
                  <p className="mt-1.5 text-[11px] text-neutral-600 dark:text-neutral-200">
        ...
        <section className="mt-6 rounded-3xl border border-neutral-200 dark:border-neutral-600 bg-white/90 dark:bg-neutral-900 p-5 sm:p-6 shadow-sm">
                ...
                className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                  step === s
                    ...
                    : index <= currentIndex
                      ...
                      : 'bg-neutral-100 text-neutral-500 dark:bg-neutral-700 dark:text-neutral-200'
                }`}
              >
          ...
          {step === 'amount' ? (
            <div className="mt-4 space-y-4">
                  ...
                  <button
                    ...
                    className={`rounded-xl border px-3 py-2 text-sm font-semibold transition-colors ${
                      amount === preset && customAmount === ''
                        ...
                        : 'border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-100'
                    }`}
                  >
              ...
              <div>
                <label className="text-xs font-semibold text-neutral-600 dark:text-neutral-100">Other amount</label>
                <input
                  type="number"
                  min={1}
                  value={customAmount}
                  ...
                  className="mt-1 w-full rounded-xl border border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3 py-2.5 text-sm text-neutral-900 dark:text-neutral-100 outline-none focus:ring-2 focus:ring-primary-500/40"
                />
              </div>
            </div>
          ) : null}
          ...
          {step === 'category' ? (
            <div className="mt-4 space-y-4">
              <div>
                <label className="text-xs font-semibold text-neutral-600 dark:text-neutral-100">Giving category</label>
                <select
                  value={categoryId}
                  onChange={(event) => {
                    const next = event.target.value
                ...
                .map((method) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <p className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">{method.label}</p>
                        <p className="text-xs text-neutral-600 dark:text-neutral-200">{method.region}</p>
                      </div>
                      {paymentMethodId === method.id ? (
                        <CheckCircle2 className="h-5 w-5 text-primary-600 dark:text-primary-200" strokeWidth={2} />
                      ) : (
                        <Wallet className="h-5 w-5 text-neutral-500 dark:text-neutral-200" strokeWidth={1.9} />
                      )}
                    </div>
                  </button>
                ))}
          ...
          {step === 'review' ? (
            <div className="mt-4 rounded-2xl border border-neutral-200 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-800 p-4">
              <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Review giving details</h3>
              <div className="mt-2 space-y-1 text-sm text-neutral-700 dark:text-neutral-100">
                <p>
              ...
              <button
                ...
                className="mt-4 rounded-xl bg-primary-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-primary-700 transition-colors"
              >
                Complete Giving
              </button>
            </div>
          ...
          {step === 'success' ? (
            <div className="mt-4 rounded-2xl border border-secondary-200 dark:border-secondary-400/40 bg-secondary-50/80 dark:bg-secondary-500/10 p-4">
              <p className="text-sm font-semibold text-secondary-900 dark:text-secondary-100">
                {data.thankYouContent.title}
              </p>
              <p className="mt-1 text-sm text-neutral-700 dark:text-neutral-100">
                {data.thankYouContent.message}
              </p>
              <p className="mt-1 text-xs text-neutral-600 dark:text-neutral-200">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2">
            {step !== 'amount' && step !== 'success' ? (
              <button
                type="button"
                onClick={goBack}
                className="rounded-xl border border-neutral-300 dark:border-neutral-600 px-4 py-2 text-sm font-semibold text-neutral-700 dark:text-neutral-100 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
              >
                Back
              </button>
            ) : null}
            {stepOrder.includes(step) ? (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-xl bg-neutral-900 dark:bg-neutral-100 px-4 py-2 text-sm font-semibold text-neutral-50 dark:text-neutral-900 disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90 transition-all"
              >
                Continue
          ...
          <div className="mt-4 rounded-2xl border border-neutral-200 dark:border-neutral-600 bg-white dark:bg-neutral-800 px-3.5 py-2.5">
            <p className="inline-flex items-center gap-1.5 text-xs font-semibold text-neutral-700 dark:text-neutral-100">
