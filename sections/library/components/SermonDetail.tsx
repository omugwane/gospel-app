...
function PrimaryButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'inline-flex items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 py-2 text-xs font-semibold text-white hover:bg-violet-700 transition-colors',
        disabled && 'bg-stone-300 text-stone-600 hover:bg-stone-300 dark:bg-stone-800 dark:text-stone-400'
      )}
    >
      {icon}
      {children}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        tone === 'amber'
          ? 'border-amber-200/80 dark:border-amber-500/30 bg-amber-50/80 dark:bg-amber-500/10 text-amber-900 dark:text-amber-100 hover:bg-amber-100/70 dark:hover:bg-amber-500/15'
          : 'border-stone-200 dark:border-stone-800 bg-white/60 dark:bg-stone-950/40 text-stone-900 dark:text-stone-100 hover:bg-stone-50 dark:hover:bg-stone-900',
        disabled && 'opacity-50'
      )}
    >
      {icon}
...
export function SermonDetail({
  ...
  if (!sermon || !sermonText) {
    return (
      <div className="mx-auto max-w-4xl rounded-3xl border border-stone-200/70 dark:border-stone-800/70 bg-white/60 dark:bg-stone-950/40 p-6">
        <div className="text-sm font-semibold text-stone-900 dark:text-stone-100">Sermon not found</div>
    ...
    <div className="w-full">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold text-stone-500 dark:text-stone-400">
              <button
                type="button"
                onClick={() => (series?.id ? onOpenSeries?.(series.id) : onOpenSermon?.(sermon.id))}
              ...
              {downloaded ? (
                <>
                  <span className="opacity-60">•</span>
                  <span className="inline-flex items-center gap-1 text-amber-900 dark:text-amber-100">
            ...
            {seriesText ? (
              <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">
                From <span className="font-semibold text-stone-800 dark:text-stone-200">{seriesText.title}</span>
              </p>
            ) : (
              <p className="mt-2 text-sm text-stone-600 dark:text-stone-400">Sermon detail</p>
            )}
          </div>
        ...
        <section className="mt-5 grid gap-3 md:grid-cols-[1fr_340px]">
          ...
          <div className="rounded-3xl border border-stone-200/70 dark:border-stone-800/70 bg-white/60 dark:bg-stone-950/40 p-5">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold tracking-wide text-stone-700 dark:text-stone-300 uppercase">
                Player
              </div>
              <div className="text-[11px] font-semibold text-stone-500 dark:text-stone-400 tabular-nums">
                {formatDuration(sermon.durationSeconds)}
              </div>
            </div>

            <div className="mt-4 rounded-2xl border border-stone-200/70 dark:border-stone-800/70 bg-[radial-gradient(circle_at_20%_20%,rgba(139,92,246,0.14),transparent_45%),radial-gradient(circle_at_85%_75%,rgba(245,158,11,0.12),transparent_55%)] p-4">
              <div className="flex items-center gap-3">
                <div className="h-11 w-11 rounded-2xl bg-violet-600 text-white flex items-center justify-center">
                  <Play className="h-5 w-5 translate-x-[1px]" strokeWidth={1.75} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-xs font-semibold text-stone-700 dark:text-stone-200">Audio-first</div>
                  <div className="mt-0.5 text-sm font-semibold text-stone-950 dark:text-stone-50 line-clamp-2">
                    {sermonText.title}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 h-1.5 rounded-full bg-stone-200 dark:bg-stone-800 overflow-hidden">
                <div className="h-full w-[34%] rounded-full bg-violet-600 dark:bg-violet-400" />
              </div>
              <div className="mt-2 flex items-center justify-between text-[11px] font-semibold text-stone-500 dark:text-stone-400 tabular-nums">
                <span>0:34</span>
        ...
        <section className="mt-4">
          <div className="rounded-3xl border border-stone-200/70 dark:border-stone-800/70 bg-white/60 dark:bg-stone-950/40 overflow-hidden">
            <div className="px-5 py-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Text className="h-4 w-4 text-stone-500 dark:text-stone-400" strokeWidth={1.75} />
                <div className="text-sm font-semibold text-stone-950 dark:text-stone-50">Transcript / Notes</div>
              </div>
              <button
                type="button"
                onClick={() => setShowTranscript((v) => !v)}
                className="text-xs font-semibold text-violet-700 dark:text-violet-200 hover:underline"
              >
            ...
            {showTranscript ? (
              hasNotes ? (
                <div className="px-5 pb-5 text-sm text-stone-700 dark:text-stone-300 leading-relaxed whitespace-pre-wrap">
                  {sermonText.transcript}
                </div>
              ) : (
                <div className="px-5 pb-5 text-sm text-stone-600 dark:text-stone-400">
              ...
              {moreLikeThis.map((s) => {
                ...
                return (
                  <button
                    ...
                    className="text-left rounded-3xl border border-stone-200/70 dark:border-stone-800/70 bg-white/60 dark:bg-stone-950/40 hover:bg-white/80 dark:hover:bg-stone-950/60 transition-colors p-5"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        {sSeriesTitle ? (
                          <div className="mt-1 text-[11px] font-semibold text-stone-600 dark:text-stone-400 line-clamp-1">
                            {sSeriesTitle}
                          </div>
                        ) : null}
                      </div>
                      <div
                        className={cn(
                          'h-10 w-10 rounded-2xl border border-stone-200/70 dark:border-stone-800/70 flex items-center justify-center',
                          isSermonSaved && 'border-violet-200 dark:border-violet-500/40 bg-violet-600/10 dark:bg-violet-400/10'
                        )}
                        aria-hidden="true"
                      >
                        {isSermonSaved ? (
